...
import { Camera, Image as ImageIcon, Loader2, X, CheckCircle, Check, FileText, Upload, Plus, Sparkles, Trash2, AlertCircle } from 'lucide-react';
...
const AddressExtractor: React.FC<AddressExtractorProps> = ({ onApplyAssignments }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
          ...
          // Base64 길이 대략 1.33배 증가 고려, 여기선 단순 길이(byte 근사)로 비교
          ...
          console.error(err);
          // 실패 시 이번에 추가하려던 파일들의 미리보기 URL 해제
